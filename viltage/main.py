...
def filter_model(soup):  # фильтрация названия модели
    model = ''.join(soup.find('div', class_='catalog_group_title').text.split())
    return model

def filter_harakteristika(soup): # фильтрация данных характеристики
...
def filter_kross(soup): # фильтрация данных кроссов
...
def filter_primenomost(soup): # фильтраци данных применимости
